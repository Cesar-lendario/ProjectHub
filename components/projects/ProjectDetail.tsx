...
const TaskCard: React.FC<{ task: Task }> = ({ task }) => {
  ...
  return (
    <Card className="mb-3 p-3">
      <h4 className="font-semibold text-slate-800 dark:text-slate-50 text-sm">{task.name}</h4>
  ...
  return (
    <div className="space-y-6">
       ...
       {(project.clientName || project.clientEmail) && (
           <Card>
               <h3 className="text-xl font-bold text-slate-800 dark:text-slate-50 mb-4">Informações de Contato</h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                   <div>
                       <p className="font-semibold text-slate-500 dark:text-slate-400">Nome do Contato</p>
                       <p className="text-slate-800 dark:text-slate-50">{project.clientName || 'Não informado'}</p>
                   </div>
                   <div>
                       <p className="font-semibold text-slate-500 dark:text-slate-400">Email de Contato</p>
                       {project.clientEmail ? (
                         <a href={`mailto:${project.clientEmail}`} className="text-indigo-600 hover:underline">
                           {project.clientEmail}
                         </a>
                       ) : (
                         <p className="text-slate-800 dark:text-slate-50">Não informado</p>
                       )}
                   </div>
               </div>
           </Card>
        ...
        <div>
          <h3 className="text-xl font-bold text-slate-800 dark:text-slate-50 mb-4">Quadro de Tarefas</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {columns.map(status => (
              <div key={status} className="bg-slate-100 dark:bg-slate-700/50 rounded-lg p-3">
                <div className="flex items-center mb-4">
                    <span className={`w-3 h-3 rounded-full mr-2 ${getStatusColor(status)}`}></span>
                    <h4 className="font-semibold text-slate-700 dark:text-slate-200">{status}</h4>
                    <span className="ml-auto text-sm font-medium bg-white text-slate-600 dark:text-slate-300 rounded-full px-2 py-0.5">
