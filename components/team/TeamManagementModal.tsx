    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center" aria-modal="true" role="dialog">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl m-4 max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-xl font-bold text-slate-800">Gerenciar Equipe: {project.name}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {/* Add user section */}
          <div>
            <h3 className="text-lg font-medium text-slate-800 mb-2">Adicionar Membro</h3>
          ...
          <div>
            <h3 className="text-lg font-medium text-slate-800 mb-2">Membros Atuais</h3>
                ...
                <div key={user.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-md">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">{user.function}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => handleRemoveUser(user.id)} disabled={isLoading} className="p-2 text-slate-500 hover:text-red-600 hover:bg-red-100 rounded-full">
                        <TrashIcon className="h-5 w-5"/>
                    </button>
                  </div>
                </div>
              ))}
              {project.team.length === 0 && <p className="text-slate-500 text-center py-4">Nenhum membro na equipe deste projeto.</p>}
            </div>
          </div>
        </div>
