    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center" aria-modal="true" role="dialog">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg m-4 max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 text-sm">
            <p className="text-slate-600">{task.description || 'Nenhuma descrição fornecida.'}</p>
            <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                <div>
                    <p className="font-semibold text-slate-500">Projeto</p>
                    <p className="text-slate-800">{task.projectName}</p>
                </div>
                <div>
                    <p className="font-semibold text-slate-500">Responsável</p>
                    <div className="flex items-center gap-2 mt-1">
                        {task.assignee ? (
                            ...
                            <span className="text-slate-800">Não atribuído</span>
                        )}
                    </div>
                </div>
                 <div>
                    <p className="font-semibold text-slate-500">Data de Vencimento</p>
                    <p className="text-slate-800">{new Date(task.dueDate).toLocaleDateString('pt-BR')}</p>
                </div>
                <div>
                    <p className="font-semibold text-slate-500">Prioridade</p>
                    <p className="text-slate-800">{task.priority}</p>
                </div>
                 <div>
                    <p className="font-semibold text-slate-500">Status</p>
                    <p className="text-slate-800">{task.status}</p>
                </div>
                <div>
                    <p className="font-semibold text-slate-500">Duração</p>
                    <p className="text-slate-800">{task.duration} dia(s)</p>
                </div>
            </div>
        </div>
