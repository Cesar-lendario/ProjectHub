...
-- VERSÃO CORRIGIDA - Conversão de tipos UUID/TEXT
-- =====================================================
...
USING (
  ...
  EXISTS (
    ...
    WHERE pt.project_id = tasks.project_id
      AND u.auth_id = auth.uid()  -- SEM conversão, ambos são UUID
    LIMIT 1
  )
  OR
  ...
  EXISTS (
    SELECT 1 
    FROM public.users u
    WHERE u.auth_id = auth.uid()  -- SEM conversão
      AND u.role = 'admin'
    LIMIT 1
  )
);
...
WITH CHECK (
  ...
  EXISTS (
    ...
    JOIN public.users u ON u.id = pt.user_id
    WHERE pt.project_id = tasks.project_id
      AND u.auth_id = auth.uid()  -- SEM conversão
