...
export const TasksService = {
  ...
  async update(id: string, task: TaskUpdate) {
    ...
    try {
      ...
      // Timeout de 45 segundos (aumentado para evitar falsos positivos em redes lentas)
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => {
          reject(new Error('A operação demorou mais que o esperado (45s). Isso pode ocorrer por instabilidade na internet ou lentidão momentânea no servidor. A tarefa pode ter sido salva mesmo com este aviso.'));
        }, 45000);
      });
